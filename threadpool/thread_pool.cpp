...
thread_pool_t* thread_pool_init()
{
	...
	if(tp == NULL) fprintf(stderr, "thread_pool_init: calloc failed!\n");
...
void thread_pool_destroy(thread_pool_t *tp)
{
    ...
    {
        lock = 1;
        if (thread_task_post(tp, &task) != T_OK) return;
...
{
    if (thread_mutex_lock(&tp->mtx) != T_OK) return T_ERROR;
...
static void* thread_pool_cycle(void *data)
{
    ...
    for ( ; ; )
    {
        if (thread_mutex_lock(&tp->mtx) != T_OK) return NULL;
        ...
        if (tp->queue.first == NULL) tp->queue.last = &tp->queue.first;
		
        if (thread_mutex_unlock(&tp->mtx) != T_OK) return NULL;
